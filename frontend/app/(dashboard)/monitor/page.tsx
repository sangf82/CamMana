...
export default function MonitorPage() {
  ...
  const [activeCameras, setActiveCameras] = useState<Record<string, string>>({}) // savedId -> activeId
  ...
  const mainCamera = filteredCameras.find(c => c.type === 'LPR') || filteredCameras[0]
  const secondaryCamera = filteredCameras.find(c => c.type !== 'LPR') || filteredCameras[1]
  ...
  return (
    <div className="h-full flex flex-col p-1 gap-1 overflow-hidden">
      ...
      <div className="flex-1 flex gap-1 min-h-0">
        ...
        <div className="w-10 flex flex-col gap-1 bg-card border border-border rounded-lg p-1 overflow-y-auto shrink-0">
           {filteredCameras.length > 0 ? (
             filteredCameras.map((cam, idx) => (
               <button 
                 key={cam.id}
                 className="w-8 h-8 bg-muted rounded flex items-center justify-center text-[10px] font-bold text-muted-foreground hover:bg-primary hover:text-black transition-colors"
                 title={cam.name}
               >
                 {idx + 1}
               </button>
               ...
               <div className="text-[8px] text-center text-muted-foreground pt-2">No Cam</div>
           )}
        </div>
        ...
        <div className="flex-1 flex flex-col gap-1">
          ...
          <div className="flex items-center justify-between bg-card border border-border px-2 py-1 rounded-lg">
             ...
             <div className="flex items-center gap-2 shrink-0">
                <button 
                  ...
                  className={`p-1.5 rounded transition-colors ${viewMode === 'focus' ? 'bg-accent text-white' : 'text-muted-foreground hover:bg-muted'}`}
                ...
                <button 
                  ...
                  className={`p-1.5 rounded transition-colors ${viewMode === 'grid' ? 'bg-accent text-white' : 'text-muted-foreground hover:bg-muted'}`}
                  title="Chế độ Lưới"
                >
                  <GridView />
                </button>
          ...
          {/* Videos */}
          <div className={`flex-1 min-h-0 bg-transparent rounded-lg overflow-hidden ${viewMode === 'grid' ? 'grid grid-cols-2 grid-rows-2 gap-1' : 'flex gap-1'}`}>
              {!currentGate ? (
                ...
                <div className="h-full flex flex-col overflow-hidden">
                  {/* Video Area - centered within container */}
                  <div className="w-full flex-1 min-h-0 flex items-center justify-center">
                    <div 
                      className="relative bg-black rounded-lg border border-border overflow-hidden"
                      style={{ height: '100%', maxHeight: '100%', aspectRatio: '16/9' }}
                    >
                      {mainCamera && isConnecting(mainCamera) && (
                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black">
                          <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4" />
                          <span className="text-sm text-muted-foreground">Đang kết nối {mainCamera.name}...</span>
                        </div>
                      )}
                      <VideoPlayer 
                    ...
                    <div className="shrink-0 mt-1 bg-card border border-border rounded-lg px-3 py-2 flex items-center justify-between w-full">
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Mã Camera</span>
                          <span className="text-sm font-bold text-primary">{mainCamera.cam_id || 'N/A'}</span>
                        </div>
                        <div className="w-px h-6 bg-border" />
                        <div className="flex flex-col">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Độ phân giải</span>
                          <span className="text-sm font-medium text-foreground font-mono">{streamInfo?.resolution || 'N/A'}</span>
                        </div>
                        <div className="w-px h-6 bg-border" />
                        <div className="flex flex-col">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">FPS</span>
                          <span className="text-sm font-medium text-foreground font-mono">{streamInfo?.fps || 0}</span>
                        </div>
                        <div className="w-px h-6 bg-border" />
                        <div className="flex flex-col">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Loại Camera</span>
                          <span className="text-sm font-medium text-foreground">{mainCamera.type || 'N/A'}</span>
                        </div>
                        <div className="w-px h-6 bg-border" />
                        <div className="flex flex-col">
                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Thương hiệu</span>
                          <span className="text-sm font-medium text-foreground">{mainCamera.brand || 'N/A'}</span>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (!activeId) return
                          try {
                            const res = await fetch(`/api/cameras/${activeId}/capture`, { method: 'POST' })
                            if (res.ok) {
                              const data = await res.json()
                              if (data.success) {
                ...
                filteredCameras.map(cam => (
                  <div key={cam.id} className="relative h-full">
                    {isConnecting(cam) && (
        ...
        <div className="w-72 bg-card border border-border rounded-lg flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-1">
             ...
             {logs.map((log, i) => (
               <div key={i} className="p-2 rounded bg-muted/30 border border-border/50 text-muted-foreground animate-in fade-in slide-in-from-right-2 duration-300">
                 {log}
               </div>
             ))}
             {logs.length > 0 && <div className="animate-pulse text-primary text-xs mt-2">Running...</div>}
          </div>

           {/* CONTROL PANEL */}
           <div className="p-3 border-t border-border bg-muted/10 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-muted-foreground">Tự động phát hiện</span>
                <button 
                  onClick={() => setIsAutoDetect(!isAutoDetect)}
                  className={`flex items-center gap-1 transition-colors ${isAutoDetect ? 'text-primary' : 'text-muted-foreground'}`}
                >
                   <span className="text-[10px] font-bold">{isAutoDetect ? 'ON' : 'OFF'}</span>
                   {isAutoDetect ? <ToggleOn fontSize="large" /> : <ToggleOff fontSize="large" />}
                </button>
              </div>
      ...
      <div className="h-48 bg-card border border-border rounded-lg p-4 flex gap-6 shrink-0 shadow-lg">
         ...
         <div className="flex-1 flex gap-8">
            <div className="flex-1 space-y-3">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-muted/30 p-2 rounded border border-border/50">
                    <span className="block text-xs text-muted-foreground mb-1">Biển số</span>
                    <span className="text-xl font-mono font-bold text-white tracking-widest">---</span>
                  </div>
                  <div className="bg-muted/30 p-2 rounded border border-border/50">
                    <span className="block text-xs text-muted-foreground mb-1">Độ tin cậy</span>
                    <span className="text-sm font-bold text-green-500">---</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Palette fontSize="small" className="text-muted-foreground" />
                    <div>
                      <span className="block text-[10px] text-muted-foreground">Màu xe</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="block text-[10px] text-muted-foreground">Số trục/bánh</span>
                      <span className="text-sm font-medium">---</span>
                    </div>
                  </div>
               </div>
            ...
            <div className="flex-1 space-y-3 opacity-50"> 
               <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Dữ liệu đăng ký</h4>
