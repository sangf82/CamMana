...
import { FilterList, Download, Search, Close, ExpandMore, ChevronRight, ChevronLeft, ExpandLess } from '@mui/icons-material'
import { toast } from 'sonner'

// --- Extended Mock Data ---
// --- Large Mock Data Generator ---
const generateMockData = (count: number) => {
  const gates = ['Cổng Nam (Vào)', 'Trạm Cân', 'Cổng Bắc (Ra)']
  const statusKeys = ['đang vào', 'đã vào', 'đang cân', 'đang ra', 'đã ra']
  const verifyKeys = ['đã xác nhận', 'ngoài danh sách']
  
  return Array.from({ length: count }, (_, i) => {
    const volStd = (Math.random() * 20 + 10).toFixed(1)
    const isOut = Math.random() > 0.3
    const status = statusKeys[Math.floor(Math.random() * statusKeys.length)]
    
    return {
      id: i + 1,
      plate: `${Math.floor(Math.random() * 90 + 10)}${['A', 'C', 'B', 'H'][Math.floor(Math.random() * 4)]}-${Math.floor(Math.random() * 900 + 100)}.${Math.floor(Math.random() * 90 + 10)}`,
      location: gates[Math.floor(Math.random() * gates.length)],
      time_in: `${String(Math.floor(Math.random() * 6) + 8).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}`,
      time_out: isOut ? `${String(Math.floor(Math.random() * 6) + 14).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}` : '---',
      vol_std: volStd,
      vol_measured: (parseFloat(volStd) - (Math.random() * 0.5)).toFixed(1),
      status: status,
      verify: verifyKeys[Math.floor(Math.random() * verifyKeys.length)],
      note: Math.random() > 0.5 ? 'Xe được cấp phép đặc biệt bởi ban quản tự, ưu tiên xử lý nhanh trong khung giờ cao điểm để tránh ùn tắc.' : 'Ghi chú kiểm tra định kỳ: Xe đạt chuẩn, không có dấu hiệu vi phạm quy định vận chuyển của khu vực.'
    }
  })
}

const MOCK_DATA = generateMockData(350)
...
const VERIFY_MAP: Record<string, { label: string, color: string }> = {
  'đã xác nhận': { label: 'Đã xác nhận', color: 'bg-emerald-500/20 text-emerald-400' },
  'ngoài danh sách': { label: 'Ngoài DS', color: 'bg-rose-500/20 text-rose-400' },
}
...
export default function HistoryPage() {
  ...
  const gates = ['All', ...Array.from(new Set(MOCK_DATA.map(d => d.location)))]
  ...
  const columns = [
    ...
    { header: 'Thể tích tiêu chuẩn (m³)', accessorKey: 'vol_std', width: '150px' },
    { header: 'Thể tích đo được (m³)', accessorKey: 'vol_measured', width: '150px' },
    { 
      header: 'Trạng thái', 
      width: '120px',
      render: (row: any) => {
        const config = STATUS_MAP[row.status] || { label: row.status, color: 'bg-muted text-muted-foreground' }
        return (
          <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold ${config.color}`}>
            {config.label}
          </span>
        )
      }
    ...
    { 
      ...
      width: '120px',
      render: (row: any) => {
        const config = VERIFY_MAP[row.verify] || { label: row.verify, color: 'bg-muted text-muted-foreground' }
        return (
          <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold ${config.color} whitespace-nowrap`}>
            {config.label}
          </span>
        )
      }
  ...
  const handleExportExcel = () => {
    ...
    link.setAttribute('download', `Lich_su_ra_vao_${today.replace(/\//g, '-')}.csv`)
    ...
    <div className="h-full flex flex-col p-6 gap-2 overflow-hidden">
         ...
         <div className="flex items-center gap-3">
            ...
            <button 
                ...
                className={`pl-3 pr-1 py-2 border rounded-md text-sm font-bold flex items-center gap-1 transition-all shadow-sm
                    ${showFilters || filterGate !== 'All' || filterStatus !== 'All' 
      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-6 py-2 px-4 bg-muted/20 border border-border rounded-lg animate-in fade-in slide-in-from-top-2 duration-200">
            {/* Location Selector */}
            <div className="flex items-center gap-3">
                <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest whitespace-nowrap">Vị trí (Gate)</label>
                <div className="relative">
                    <select 
                        ...
                        onChange={(e) => { setPendingFilterGate(e.target.value); setIsGateOpen(false); }}
                        onFocus={() => setIsGateOpen(true)}
                        onBlur={() => setIsGateOpen(false)}
                    >
                        {gates.map(g => <option key={g} value={g}>{g === 'All' ? 'Tất cả vị trí' : g}</option>)}
                    </select>
                    <div className="absolute inset-y-0 right-2 flex items-center pointer-events-none text-muted-foreground">
                        <ChevronLeft 
                            fontSize="small" 
                            className={`opacity-70 transition-transform duration-200 ${isGateOpen ? '-rotate-90' : ''}`} 
                        />
                    </div>
                </div>
            </div>
            ...
            {/* Status Selector */}
            <div className="flex items-center gap-3">
                <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest whitespace-nowrap">Trạng thái</label>
                <div className="relative">
                    <select 
                        className="appearance-none bg-background border border-border rounded-md pl-3 pr-10 py-1.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none min-w-[180px] cursor-pointer"
                        value={pendingFilterStatus}
                        onChange={(e) => { setPendingFilterStatus(e.target.value); setIsStatusOpen(false); }}
                        onFocus={() => setIsStatusOpen(true)}
                        onBlur={() => setIsStatusOpen(false)}
                    >
                        {statuses.map(s => <option key={s} value={s}>{s === 'All' ? 'Tất cả trạng thái' : STATUS_MAP[s]?.label}</option>)}
                    </select>
                    <div className="absolute inset-y-0 right-2 flex items-center pointer-events-none text-muted-foreground">
                        <ChevronLeft 
                            fontSize="small" 
                            className={`opacity-70 transition-transform duration-200 ${isStatusOpen ? '-rotate-90' : ''}`} 
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 ml-auto">
                {(filterGate !== 'All' || filterStatus !== 'All' || searchTerm !== '') && (
                    <button 
                        onClick={() => { setFilterGate('All'); setFilterStatus('All'); setPendingFilterGate('All'); setPendingFilterStatus('All'); setSearchTerm(''); }}
              ...
              <tr>
                {columns.map((col, idx) => (
                  <th key={idx} className="px-4 py-3 whitespace-nowrap" style={{ width: col.width || 'auto' }}>
                    {typeof (col.header as any) === 'function' ? (col.header as any)() : col.header}
                  </th>
                ))}
              </tr>
            </thead>
